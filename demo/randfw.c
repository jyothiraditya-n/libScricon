...
static void help(const char *name) {
	printf("Usage: %s [-no-colour] COLS ROWS [CHARS] [FG_COLS] [BG_COLS] [SLOWDOWN]\n", name);
	printf("       %s OPTION\n\n", name);
	...
	printf("[*_COLS] are the optional lists of colours to pick from "
...
int main(int argc, char **argv) {
	...
	int ret = sscanf(argv[1 + (int) no_colour], "%zu", &width);
	if(ret != 1) inp_err(argv[0]);

	ret = sscanf(argv[2 + (int) no_colour], "%zu", &height);
	if(ret != 1) inp_err(argv[0]);
	height--;
	...
	const char *chrs = argv[3 + (int) no_colour];
	...
	const char *fgs = argv[4 + (int) no_colour];
	...
	const char *bgs = argv[5 + (int) no_colour];
	...
	if(argc > 6 + (int) no_colour) {
		int ret = sscanf(argv[6 + (int) no_colour], "%zu", &slowdown);
		if(ret != 1) inp_err(argv[0]);
	}
